...
public class StorageUtils {
	...
	public static int simpleFolderMatch(TileEntityRFC tile, ItemStack stack) {
		...
		for (int i = 0; i < tile.getInventory().getSlots(); i++) {
			...
			if (loopinv != null && (tile.getInventory().getTrueStackInSlot(i).getItemDamage() == 5 && ItemStack.areItemStackTagsEqual(stack, loopinv)))
				return i;
			
			if (loopinv != null && tile.getInventory().getTrueStackInSlot(i).getItemDamage() != 5 && simpleMatch(stack, loopinv))
				return i;
		}
		return -1;
	}
	...
	public static void addStackManually(TileEntityRFC tile, EntityPlayer player, ItemStack stack) {
		...
		for (int i = 0; i < tile.getInventory().getSlots(); i++) {
			...
			if ((tile.getInventory().getTrueStackInSlot(i) != null && tile.getInventory().getTrueStackInSlot(i).getItemDamage() == 2)) {
			...
			if (loopinv != null && (loopinv.getItem() != RFCItems.emptyFolder || loopinv.getItem() != RFCItems.folder))
			{
				...
				for (int j = 0; j < tile.getInventory().getSlots(); j++) {
					...
					if (tilestack != null && ItemFolder.getObject(tilestack) instanceof ItemStack)
					{
						...
						if (tilestack != null && tilestack.getItemDamage() != 5 && ItemStack.areItemsEqual(folderstack, loopinv))
						{
							ItemFolder.add(tilestack, loopinv.stackSize);
							player.inventory.setInventorySlotContents(i, null);
							consume = true;
