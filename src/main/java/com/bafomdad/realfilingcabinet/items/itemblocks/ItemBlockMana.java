...
public class ItemBlockMana extends ItemBlock {
	...
	private void listItems(ItemStack stack, List<String> list) {
		...
		for (int i = 0; i < tagList.tagCount(); i++) {
			...
			if (!folder.isEmpty() && folder.getItem() instanceof IManaItem) {
				int manaCount = ((IManaItem)folder.getItem()).getMana(folder);
				list.add(new TextComponentTranslation(TextHelper.formatMana(manaCount)).getFormattedText());
			}
		}
	}
}
